      ...
      <div className="teaching-main">
        ...
        <div className="teaching-content">
          ...
          <Row gutter={[16, 16]} className="mt-16">
            ...
            <Col span={16}><div className="data-panel"><div className="panel-title">考试列表</div><Table dataSource={mockExams} rowKey="id" size="small" pagination={false} columns={[{ title: "考试名称", dataIndex: "title", key: "title" }, { title: "课程", dataIndex: "course", key: "course" }, { title: "日期", dataIndex: "date", key: "date" }, { title: "已阅/总数", key: "graded", render: (_: unknown, r: { gradedCount: number; totalStudents: number }) => r.gradedCount + "/" + r.totalStudents }, { title: "平均分", dataIndex: "avgScore", key: "avgScore", render: (v: number) => v > 0 ? <span style={{ color: "#00d4ff", fontWeight: 600 }}>{v}</span> : "-" }, { title: "状态", dataIndex: "status", key: "status", render: (s: string) => <Tag color={s === "completed" ? "#00e676" : s === "grading" ? "#ff9100" : "#1677ff"}>{s === "completed" ? "已阅" : s === "grading" ? "阅卷中" : "待考试"}</Tag> }, 
                                                                                                                                                                                  { title: "操作", key: "action", render: (_: any, record: any) => if (record.status === "completed") return <Button size="small">查看成绩</Button> return <Button type="primary" size="small" disabled={record.status === "upcoming"}>开始阅卷</Button> }]} /></div></Col>
          </Row>
        </div>
      </div>
    </div>
